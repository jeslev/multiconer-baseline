...
def objective(trial):
    ...
    train_data = get_reader(file_path=sg.train, target_vocab=get_tagset(sg.iob_tagging), encoder_model=sg.encoder_model, max_instances=sg.max_instances, max_length=sg.max_length, train=True)
    ...
    batch_size=4#trial.suggest_categorical('batch',[16,32])
    ...
    model = create_model(train_data=train_data, dev_data=dev_data, tag_to_id=train_data.get_target_vocab(),
                         ...
                         encoder_model=sg.encoder_model, num_gpus=sg.gpus, model_type=sg.version_model)
